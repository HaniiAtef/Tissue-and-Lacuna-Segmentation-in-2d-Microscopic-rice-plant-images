...
def pad_slice_rows(rows, target_rows, panel_width=512, panel_height=341, label_width=120):
    blank_row = np.hstack([
        ...
        np.ones((panel_height, panel_width*2, 3), dtype=np.uint8)*255
    ])
    rows_padded = rows[:]
...
class SegformerPredictor:
    ...
    def run_prediction(self):
        ...
        return ar_dir, csv_path

# ------------------------
# VISUALIZATION FUNCTION
# ------------------------
def build_tiff(raw_dir, mask_dir, csv_path, output_path, overlay_alpha=0.3):
    ...
    for _, row in tqdm(df_sorted.iterrows(), total=len(df_sorted), desc="Building panels"):
        ...
        if raw_img is None:
            print(f"Failed to read raw image: {raw_path}, skipping.")
            continue
        ...
        mask_resized = cv2.resize((mask>127).astype(np.uint8),(512,341),interpolation=cv2.INTER_NEAREST)

        overlay = overlay_mask(base_rgb.copy(), mask_resized, color=(255,0,0), alpha=overlay_alpha)
...
def main():
    ...
    parser.add_argument('--images', type=str, default='/home/hmohamed/Documents/test_github/data/inference_images', help='Path to image or folder of images')
    ...
    mask_dir, csv_path = predictor.run_prediction()
    build_tiff(os.path.join(args.output,'raw_images'), mask_dir, csv_path, out_tif, overlay_alpha=args.alpha)
